...
// A live context retains its own scratch name across switches and migration.
thread_local GLuint fallback_restart_ibo = 0;
GLuint& restart_ibo() {
    return g_current_ctx ? g_current_ctx->restart_scratch_buffer : fallback_restart_ibo;
}
...
bool mg_draw_elements_restart(GLenum mode, GLsizei count, GLenum type, const void* indices, GLint basevertex,
    ...
    // Grown but never shrunk, and private to the thread rewriting this stream.
    // Only the first `count` elements are written and uploaded, so whatever a
    ...
    GLuint& g_restart_ibo = restart_ibo();
